...
let normalize a=
  let d=sqrt (scal a a) in {x=a.x/.d;y=a.y/.d}
...
let solve pa na pb nb=
  ...
  if abs_float d < epsilon then
    ...
    {x=(determ ca cb na.y nb.y)/.d;y=(determ na.x nb.x ca cb)/.d}
...
let intersect p v cadre =
  if !debug then (Printf.printf "p=%f %f\n" p.x p.y; flush stdout;
		  Printf.printf "v=%f %f\n" v.x v.y; flush stdout);
  ...
  let rec inter p v cadre sw oq l =
	...
	if sw then 
	  ...
	  if inn then
	    ...
	    inter p v tl sw q l in
  match cadre with
    [] -> [] 
  | hd::tl -> inter p v tl (inside hd p v) hd []
...
let rec cross_segconv a b convex=
  match convex with
    c::d::tl ->
     cross_segs a b c d || cross_segconv a b  (d::tl)
  | _ -> false
...
let projection p convex =
  let rec rec_project p convex oq r rp=
    ...
    | q::tl -> 
      ...
      if s<r then rec_project p tl q s sp
      else rec_project p tl q r rp in
  match convex with
    [] -> failwith "convex vide"
  | oq::_ ->
    rec_project p convex oq (1./.0.) oq


let projmax p convex =
  snd  (List.fold_left (fun (d,v) q -> let t=pscal p q p in if t>d then (t,q) else (d,v)) (0.,p) convex)
...
let extremes a l =
  ...
  | hd :: _ ->
     let v0=diff hd a in
     List.fold_left (fun (d,g,vd,vg) p ->
         let vc=diff p a in
         let pd=vect vd vc in
         if pd < 0. then (p,g,vc,vg)
         else
           let pg=vect vg vc in
           if pg > 0. then (d,p,vd,vc)
           else (d,g,vd,vg)) (hd,hd,v0,v0) l
           
   
let box=
  List.rev  [{x= -2.000000;y= -10.000000};

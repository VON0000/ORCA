...
let points =
  let tab = Array.init dim (fun _ -> { x = 0.; y = 0. }) in
  for i = 0 to dim - 1 do
    try
      while true do
        tab.(i) <-
        ...
        try
          for j = 0 to i - 1 do
            if dist2 tab.(i) tab.(j) < sep *. norme2 then raise Exit
          done;
          raise Good
        with Exit -> ()
      done
...
let speeds =
  Array.init dim (fun i ->
      let angle =
        atan2 (dests.(i).y -. points.(i).y) (dests.(i).x -. points.(i).x)
      in
  ...
  for i = 0 to dim - 1 do
    if active.(i) then (
      ...
      Printf.fprintf fichs "%f %f\n%f %f\n\n" points.(i).x points.(i).y
        (points.(i).x +. (coeff *. speeds.(i).x))
        (points.(i).y +. (coeff *. speeds.(i).y));
      flush fichs;
      Printf.fprintf fichns "%f %f\n%f %f\n\n" points.(i).x points.(i).y
        (points.(i).x +. (coeff *. newspeeds.(i).x))
        (points.(i).y +. (coeff *. newspeeds.(i).y));
      flush fichns;
      List.iter
        (fun { x; y } ->
  ...
  for i = 0 to dim - 1 do
    if active.(i) then (
      ...
      if
        not
          (Array.fold_left
             (fun tf o -> tf || is_inside { x = newx; y = newy } o)
             false obstacle)
      then (
        points.(i).x <- newx;
        points.(i).y <- newy)
      else Printf.printf "ENTREE DANS OBSTACLE\n";
      if dist2 points.(i) dests.(i) < 4. *. norme2 then (
        active.(i) <- false;
        incr fin);
      Printf.fprintf fichmem "%f %f\n\n" points.(i).x points.(i).y;
      flush fichmem)
...
let vectu pa sa pb sb tau =
  ...
  let region_neutre vrbis nvrbis =
    ...
    let res =
      {
        x = rayon *. vrbis.x /. nvrbis /. 2.;
        y = rayon *. vrbis.y /. nvrbis /. 2.;
      }
    in
    (*    Printf.printf "NEUTRE";*)
    if rayon >= 0. then Some (res, res) else Some (res, opp res)
  in
  ...
  if npr < 2. *. norme then (
    ...
    if pvect p1 prbis vr > 0. && pvect vr prbis pr >= 0. then (
      ...
      in
      Some
        ( { x = nvrsa *. u.x; y = nvrsa *. u.y },
          { x = abs_nvrsa *. u.x; y = abs_nvrsa *. u.y } ))
...
let speedbox _point speed _speedopt sizelong _sizelat _ns =
  ...
  let box =
    Array.init (nb + 1) (fun i ->
        let na = angle +. (float i *. 2. *. pi /. nbf) in
...
let cut i initbox speed constraints =
  ...
  if !debug then (
    List.iter
      (fun (nv, _qv, _tf) -> Printf.printf "%d %f %f\n" i nv.x nv.y)
      constraints;
    Printf.printf "\n";
    flush stdout);
       ...
       try
         List.iter
           (fun (nv, qv, tf) ->
             ...
             let p =
               {
                 x = (speed.x +. nv.x -. if tf then !delta *. uv.x else 0.);
                 y = (speed.y +. nv.y -. if tf then !delta *. uv.y else 0.);
               }
             in
...
let project i speed targetbox =
  if !debug then (
    Printf.printf "PROJECT\n";
    flush stdout);
  if is_inside speed targetbox then (
    if !debug then (
      Printf.printf "is_inside %d %f %f\n" i speed.x speed.y;
      flush stdout);
    speed)
  else

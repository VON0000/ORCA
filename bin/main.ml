  ...
  while !fin < dim do
    ...
    let speedopt =
      Array.init dim (fun i ->
          ...
          { x = speed *. cos cap; y = speed *. sin cap })
    in
    ...
    for i = 0 to dim - 1 do
      if active.(i) then (
                ...
                if scal pnv pnv > epsilon then (
                  if !debug then (
                    Printf.printf "%d %f %f %d %f %f %f %f\n" i points.(i).x
                      points.(i).y j points.(j).x points.(j).y pnv.x pnv.y;
                    flush stdout);
        ...
        let previopttau =
          {
            x = points.(i).x +. (tau *. speedopt.(i).x);
            y = points.(i).y +. (tau *. speedopt.(i).y);
          }
        in
        let previ =
          {
            x = points.(i).x +. (pas *. speeds.(i).x);
            y = points.(i).y +. (pas *. speeds.(i).y);
          }
        in
        let previtau =
          {
            x = points.(i).x +. (tau *. speeds.(i).x);
            y = points.(i).y +. (tau *. speeds.(i).y);
          }
        in
        for j = 0 to Array.length obstacle - 1 do
          if
            cross_segconv points.(i) previopttau obstacle.(j)
          ...
          then (
            ...
            let extd =
              { x = cos (angled -. (pi /. 2.)); y = sin (angled -. (pi /. 2.)) }
            in
            let pg = proj_pt_vec previ points.(i) vg in
            let _ng, angleg = normangle vg in
            let extg =
              { x = cos (angleg +. (pi /. 2.)); y = sin (angleg +. (pi /. 2.)) }
            in
            let p, ext =
              if scal pd pd < scal pg pg then (pd, extd) else (pg, extg)
            in
    ...
    for i = 0 to dim - 1 do
      let initbox =
        speedbox points.(i) speeds.(i) speedopt.(i) sizelong sizelat nspeeds.(i)
      in
      ...
      let newspeed =
        ...
        if sqrt (scal ns ns) > speed /. 5. then ns
          ...
          let ns = project i (rotate 1. speeds.(i) 0.) targetbox in
          ns
      in
      if !debug then (
        Printf.printf "newspeed=%f %f\n" newspeed.x newspeed.y;
        flush stdout);
      oldspeeds.(i) <- speeds.(i);
      speeds.(i) <- newspeed;
      boites.(i) <- targetbox
    done;
    if !t mod step = 0 then printpsb false points speedopt speeds boites;
